...
Every task got a review pass with fresh context before it was committed. On the payment service, that pass found a bug in code the same tool had written minutes earlier.

The bug: two requests using the same idempotency key on different bookings both pass the "has this key been used" check, because neither can see the other's uncommitted row. The second one then hits the unique index and returns a raw database error as a 500, and its card authorization is left hanging.
...
- Track test coverage as it is written. The spec predicted six cases. The suite landed at 16 because several cases split and a third concurrency test was added. Good outcome, but I had to work out what was actually covered at the end instead of knowing as I went.
...
- Did not run the `pool max: 1` version of that check. Six actors on one connection still add up correctly, so it would pass and prove nothing.
